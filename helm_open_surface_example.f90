...
!
      ...
      ipv = 0
      call prinf('nptuse=*',nptuse,1)
      call prinf('ipatch_id=*',ipatch_id,nptuse)
      call prin2('uvs_src=*',uvs_src,2*nptuse)
      ...
      do i=1,nptuse
        fints(i) = 0
        rr = sqrt(srcvals(1,i)**2 + srcvals(2,i)**2)
        print *, i, sqrt(srcvals(1,i)**2 + srcvals(2,i)**2)
        do j=1,npts
          fints(i) = fints(i) + amat(j,i)
        enddo
        write(34,*) real(fints(i)), imag(fints(i)), rr
      enddo
